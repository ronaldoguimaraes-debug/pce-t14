...
const FALLBACK_META = {version:'3.0.0',capturedAt:'2026-03-27T18:00:00.000Z',totalRows:0,isOffline:true,source:'static_embedded'};
// ─── v49.6: NETWORKING PV — Noite de Networking (v49.10: dedupe por CPF/nome) ───
const NETWORKING_PV_FALLBACK = []; /* Segurança 25/09/2026: dados de aluno removidos do código (nome, CPF, e-mail). As listas vêm só dos endpoints em tempo de execução. Engenharia e fundação: Ronaldo Ferreira */
...
const FALLBACK = []; /* Segurança 25/09/2026: dados de aluno removidos do código (nome, CPF, e-mail). As listas vêm só dos endpoints em tempo de execução. Engenharia e fundação: Ronaldo Ferreira */
const PERFIL = []; /* Segurança 25/09/2026: dados de aluno removidos do código (nome, CPF, e-mail). As listas vêm só dos endpoints em tempo de execução. Engenharia e fundação: Ronaldo Ferreira */
const FAQ_DATA = [
...
const G='#63F858',G2='#243d26',G3='#4bc943',AM='#f5c542',RD='#ff5f5f',BL='rgba(99,248,88,.5)',CR='#EAE5E1';
const PCE_CAT=[
  G,
  'rgba(99,248,88,.62)',
  'rgba(99,248,88,.40)',
...
const TOOLTIP_DEFAULTS={backgroundColor:'#1a1a18',borderColor:'rgba(99,248,88,.12)',borderWidth:1,titleColor:CR,bodyColor:'#7a7a76',titleFont:{family:'Barlow',weight:'600',size:12},bodyFont:{family:'Barlow',size:12},padding:10};
...
function renderSeatmap(label,confirmados,turmaValida){
 ...
 const svg='<svg viewBox="120 0 760 230" class="pl-svg" xmlns="http://www.w3.org/2000/svg">'+defs+room+stage+labels+seatSvg+'</svg>';
 ...
 {var _mbc=document.getElementById('month-bar');if(_mbc){var _oc=_mbc.querySelector('.mb-cap');if(_oc)_oc.remove();_mbc.insertAdjacentHTML('beforeend','<span class="mb-cap">· '+cap+' lugares</span>');}}
...
function renderViewGeral(){
  ...
  safeRender('imersao-container',()=>{
    document.getElementById('imersao-container').innerHTML=
      ...
      + turmaStats(t15, 'T15', '#63F858', 'Agosto')
      + turmaStats(t16, 'T16', '#63F858', 'Novembro')
      +'</div>';
  });
...
const mb=document.getElementById('month-bar');try{document.querySelectorAll('.faixa-vazia-turma').forEach(function(e){e.textContent=(v==='br'?'T14':v.toUpperCase());});}catch(e){}if(mb){if(months[v]){mb.innerHTML='<span class="mb-turma">'+(v==='br'?'T14':v.toUpperCase())+'</span><span class="mb-kicker">Imersão de</span><span class="mb-month">'+months[v]+'</span>';mb.style.display='flex';}else{mb.style.display='none';}}renderCurrentView();}
...
/* LOTE 1 · estados com contexto e navegacao mobile */
var PAGINAS_CONTEXTO={
  'nps':'Aqui ficam as <b>notas de satisfação</b> da imersão e dos hot seats, o ranking de palestrantes e os comentários dos mentorados.',
  'experts':'Aqui ficam as <b>notas de satisfação</b> da imersão dos Experts, com ranking de palestrantes e comentários.',
  'manual':'Aqui fica o <b>Manual Experts</b>, com a operação do evento dia a dia, materiais e links para o time.',
  'manual-pce':'Aqui fica o <b>Manual PCE</b>, com a operação do evento dia a dia, materiais e links para o time.',
  'turmas':'Aqui fica o <b>histórico das turmas</b> com os indicadores de cada imersão.'
};
function toggleMobileNav(force){
  var open=(typeof force==='boolean')?force:!document.body.classList.contains('nav-open');
  document.body.classList.toggle('nav-open',open);
  var sb=document.getElementById('sidebar');if(sb&&open)sb.classList.remove('collapsed');
}
function syncMobileTab(p){
  var map={'dash':'dash','perfil':'perfil','insights':'perfil','nps':'nps','experts':'nps','estoque':'estoque'};
  var key=map[p]||'';
  document.querySelectorAll('.mob-tab').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-mob')===key);});
}
function showLockedPage(p){
  var nome=PAGINAS_RESTRITAS[p]||'Esta área';
  window.__gatedPending=p;
  ['dash','perfil','faq','insights','manual','manual-pce','nps','experts','estoque','manutencao','admin'].forEach(function(pg){var el=document.getElementById('page-'+pg);if(el)el.classList.remove('active');});
  var lk=document.getElementById('page-locked');if(lk)lk.classList.add('active');
  var m=document.getElementById('locked-msg');if(m)m.innerHTML=(PAGINAS_CONTEXTO[p]||'Esta área contém informações restritas.')+' Entre com uma conta de editor para visualizar.';var mt0=document.getElementById('locked-title');if(mt0)mt0.textContent=nome+' · conteúdo protegido';
...
function switchPage(p){
  try{toggleMobileNav(false);syncMobileTab(p);}catch(e){}
  ...
  if(p==='turmas'){
    ...
    var mMsg=document.getElementById('manutencao-msg');if(mMsg)mMsg.innerHTML='Esta área vai reunir o <b>histórico das turmas</b> (T14, T15, T16 e futuras) com os mesmos indicadores do Dashboard: confirmados, sem retorno, ocupação e NPS lado a lado.';var mT=document.getElementById('manutencao-title');if(mT)mT.textContent='Turmas · em construção';var mC=document.getElementById('manutencao-chip');if(mC)mC.style.display='';
...
function buildPerfil() {
  ...
  safeRender('kpi-perf', () => {
    ...
    if(orgVisEl){
      ...
      const orgColors={'Sim':'#46d160','Parcialmente':'#63F858','Não':'#ff5f5f'};
    ...
    const cL2=Object.keys(confMap),cD2=cL2.map(k=>confMap[k]),cC2=['#46d160','#63F858','#ff5f5f'];
    ...
    const iaCols=[1,2,3,4,5].map(k=>k<=2?'#ff5f5f':k===3?'#63F858':'#46d160');
...
/* ========================================================================
   ...
   ======================================================================== */
#diagnostico-card{border:1px solid var(--border2);background:linear-gradient(180deg,rgba(99,248,88,.04) 0%,var(--card) 60%)}
...
.diag-leitura{margin-top:18px;padding:16px 20px;background:rgba(99,248,88,.06);border-left:3px solid var(--green);border-radius:6px}
...
.diag-leitura-ang{margin-top:12px;padding-top:12px;border-top:1px solid rgba(99,248,88,.18);font-size:11.5px;color:var(--muted);font-style:italic;line-height:1.5}
...
.wcloud span:hover{opacity:1!important}
  ...
  function renderTopFaturamento(items) {
    ...
    el.innerHTML = items.map(function(r, i) {
      ...
      const medColor = i===0 ? '#f5c542' : i===1 ? '#adada8' : i===2 ? '#4bc943' : 'var(--muted)';
...
(function() {
  ...
  function renderTipoCliFat(data) {
    ...
    const rows = lista.map(k => {
      ...
      return `<div class="ins-fx-row"><div class="ins-fx-label" style="color:${cores[k]||'#fff'};font-weight:700">${k}</div><div class="ins-fx-bar"><div class="ins-fx-fill" style="width:${w}%;background:${cores[k]||'#63F858'}"></div></div><div class="ins-fx-val">${fmtBRL(media)}<span style="font-size:9px;color:var(--muted);font-weight:500;letter-spacing:0">/ano</span></div><div class="ins-fx-count">${agg[k].count} emp.</div></div>`;
    }).join('');
  ...
  function renderMvvPerf(data) {
    ...
    if (!el) return;
    const grupos = ['Com MVV','Sem MVV'], cores = ['#63F858','#7a7a76'];

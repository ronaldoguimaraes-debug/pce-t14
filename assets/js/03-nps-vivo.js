...
(function(){
  ...
  function scoreColor(v,max){var r=v/max;return r>=0.92?'#46d160':r>=0.8?'#63F858':r>=0.6?'#f5c542':'#ff5f5f';}
  ...
  function renderHotseat(list){
    ...
    if(canvas&&_rank.length){
      var keys=['satisfacao','relevancia','conteudo'],kc={satisfacao:'#63F858',relevancia:'#46d160',conteudo:'rgba(234,229,225,.55)'};
      ...
      if(window.Chart)charts.push(new Chart(canvas,{type:'bar',
        data:{labels:_rank.map(function(e,i){return (i+1)+'\u00BA  '+(e.mentor||e.turma);}),
          datasets:useKeys.map(function(k){return {label:HS_LABELS[k]||k.replace('extra:',''),data:_rank.map(function(e){return e.metricas[k]?+e.metricas[k].avg.toFixed(2):null;}),backgroundColor:kc[k]||'#63F858',borderRadius:3,maxBarThickness:14};})},
  ...
  async function npsExportPNG(id){
    ...
    var C={
      ...
      g:_pngCss('--green','#63F858'), g2:_pngCss('--green2','#243d26'),
